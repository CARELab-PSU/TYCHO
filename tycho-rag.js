...
const TychoRAG = (() => {
  ...
  function retrieve(query, k = 4, minScore = 9.0) {
    ...
    const parts = top.map(([s, i]) => {
      const c = chunks[i];
      const loc = c.p ? ` (p. ${c.p})` : "";
      return `--- Section ${c.s}${loc} ---\n${c.t}`;
    });
    return (
      "\n\n== COURSE TEXTBOOK EXCERPTS (Kluever, Space Flight Dynamics) ==\n" +
      "The following passages were retrieved from the AERSP 450 course textbook " +
      ...
      "a concept covered below, point the student to the section number so they can " +
      "read it themselves. Do NOT quote long passages verbatim; paraphrase and guide.\n\n" +
      parts.join("\n\n")
    );
  }
