...
const TychoRAG = (() => {
  ...
  const SYN = {
    ...
    quaternion: ["euler", "parameter", "beta"],
    quaternions: ["euler", "parameter", "beta"],
    gimbal: ["singularity", "euler", "angle"],
    lock: ["singularity", "gimbal"],
    singularity: ["singular", "gimbal", "euler"],
  ...
  // Phrase-level concept routing: student colloquialisms / homonyms that
  // lexical search sends to the wrong section. Matched phrases inject
  // full-weight concept terms in the BOOK's vocabulary.
  const PHRASE_CONCEPTS = [
    [/gimbal\s*lock/, ["singularity", "euler", "angle", "kinematic", "parameter"]],
    [/quaternion/, ["euler", "parameter", "kinematic", "attitude"]],
    [/burn\b/, ["impulse", "maneuver", "deltav"]],
    [/spin\s*stab/, ["stability", "nutation", "axisymmetric"]],
    [/pork\s*chop/, ["porkchop", "heliocentric", "lambert"]],
  ];

  function tokenize(text) {
    let s = text.toLowerCase();
    for (const [re, rep] of NORMALIZE) s = s.replace(re, rep);
    const raw = s.match(/[a-z]{2,}|[a-z]\d+|\d+/g) || [];
  ...
  function retrieve(query, k = 4, minScore = 9.0) {
    ...
    // phrase-level concept routing (full-weight, book vocabulary)
    const ql = query.toLowerCase();
    for (const [re, concepts] of PHRASE_CONCEPTS) {
      if (re.test(ql)) qt = qt.concat(concepts);
    }
